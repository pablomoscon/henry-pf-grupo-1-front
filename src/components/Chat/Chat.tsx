...
const Chat = () => {
  ...
  const [hasActiveReservation, setHasActiveReservation] = useState(true);
  ...
  useEffect(() => {
    ...
    const isClientChat = pathParts.includes("client-chat");

    if (isClientChat) {
      const checkReservations = async () => {
        try {
          const response = await fetch(
            `http://localhost:3000/books/active/${user?.response?.user?.id}`,
            {
              headers: {
  ...
  }, [setCurrentChatId, user?.response?.user?.id, user?.response?.token]);
      ...
      <div className="w-full max-w-4xl h-[90%] mx-2 bg-black-dark rounded-lg shadow-lg border border-gray-700 flex flex-col">
        ...
        {!hasActiveReservation && (
          <div className="bg-red-600 text-white p-4 text-center">
            You currently do not have any reservations, so chatting is not
            available right now.
          </div>
        )}
                ...
                <div
                  className={`min-w-[20%] rounded-lg p-3 me-2 ms-2 ${
                    isSender
                      ...
                      : "bg-gray-600 text-white-basic"
                  }`}
                >
                  <p
                    className={`text-m font-bold ${
                      isReceiver ? "text-gold-soft" : ""
                    }`}
                  >
                    {message.senderName}
                  </p>
        ...
        <div className="p-4 border-t border-gray-700 bg-black-dark mt-auto">
          <div className="flex gap-2">
            <input
              ...
              onKeyPress={(e) =>
                e.key === "Enter" && hasActiveReservation && handleSend()
              }
              className="flex-1 bg-gray-700 text-white-basic rounded-lg px-4 py-2"
              placeholder={
                hasActiveReservation
                  ? "Type your message..."
                  : "Chat disabled - No active reservations"
              }
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${
                hasActiveReservation
                  ? "bg-gold-soft text-black-dark hover:bg-gold-dark"
